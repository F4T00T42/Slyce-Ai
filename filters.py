"""Hard pass/fail gates: allergen check + diet macro gates.

Reused unchanged from the original engine. Allergen detection works off
`meal.tags`; the repository layer now populates `tags` with allergen names
derived from the meal's ingredients (since the DB `Tags` column is sparse).
"""
